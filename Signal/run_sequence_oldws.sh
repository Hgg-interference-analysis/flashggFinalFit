...
if [[ $STEP == "fTest" ]] || [[ $STEP == "calcPhotonSyst" ]] || [[ $STEP == 'signalFit' ]]; then
	...
	if [[ $year == $YEAR ]] || [[ $YEAR == "all" ]]; then
	    ...
	    if [[ $STEP == "fTest" ]]; then
		python3 RunSignalScripts.py --inputConfig config_test_${year}_oldws.py --mode fTest --modeOpts "--doPlots --outdir /eos/user/r/rgargiul/www/width_pdf/plots_oldws --nProcsToFTest -1" ${DROPT}
	    elif [[ $STEP == "calcPhotonSyst" ]]; then
		python3 RunSignalScripts.py --inputConfig config_test_${year}_oldws.py --mode calcPhotonSyst ${DROPT}
	    elif [[ $STEP == 'signalFit' ]]; then
		python3 RunSignalScripts.py --inputConfig config_test_${year}_oldws.py --mode signalFit --modeOpts="--doPlots --outdir /eos/user/r/rgargiul/www/width_pdf/plots_oldws --skipSystematics" ${DROPT}
	    fi
	fi
    done
elif [[ $STEP == 'packager' ]]; then
    python RunPackager.py --cats "auto" --inputWSDir /eos/user/r/rgargiul/dataHggWidth/ws_postVBFcat_noVBFGGFmix/ --outputExt packaged_oldws --exts 2024-10-06_oldws_year2018 --year $YEAR --batch local --mergeYears
...
#    python RunPlotter.py --procs $smprocs_csv --cats "all" --years 2018 --ext packaged_oldws --outdir /eos/user/r/rgargiul/www/width_pdf/plots_oldws
...
#    	python RunPlotter.py --procs $smprocs_csv --cats $cat --years 2018 --outdir /eos/user/r/rgargiul/www/width_pdf/plots_oldws --ext packaged_oldws --translateCats ../Plots/cats.json
#    done
...
#    	python RunPlotter.py --procs $proc --cats "all" --years 2018 --ext packaged_oldws --outdir /eos/user/r/rgargiul/www/width_pdf/plots_oldws
#    done
 ...
 --ext packaged_oldws --outdir /eos/user/r/rgargiul/www/width_pdf/plots_oldws
        done
    done
