...
if [[ $STEP == "fTest" ]] || [[ $STEP == "calcPhotonSyst" ]] || [[ $STEP == 'signalFit' ]]; then
	...
	if [[ $year == $YEAR ]] || [[ $YEAR == "all" ]]; then
	    ...
	    if [[ $STEP == "fTest" ]]; then
		python3 RunSignalScripts.py --inputConfig config_test_${year}.py --mode fTest --modeOpts "--doVoigtian --skipWV --doPlots --outdir /eos/user/r/rgargiul/www/width_pdf/plots --nProcsToFTest -1" ${DROPT}
		...
		python3 RunSignalScripts.py --inputConfig config_test_${year}.py --mode signalFit --modeOpts="--useDCB --skipVertexScenarioSplit --doPlots --outdir /eos/user/r/rgargiul/www/width_pdf/plots_dcb" ${DROPT}
	    fi
	fi
    done
elif [[ $STEP == 'packager' ]]; then
    python3 RunPackager.py --cats "auto" --inputWSDir /eos/cms/store/group/phys_higgs/cmshgg/rgargiul/ws_${YEAR}/ --outputExt packaged --exts dcb-newcat-2024-05-02_year${YEAR} --year $YEAR --batch local
    ...
    python3 RunPlotter.py --procs $smprocs_csv --cats "all" --years ${YEAR} --ext packaged --outdir /eos/user/r/rgargiul/www/width_pdf/plots_dcb &
    ...
    do
    	...
    	python3 RunPlotter.py --procs $smprocs_csv --cats $cat --years ${YEAR} --outdir /eos/user/r/rgargiul/www/width_pdf/plots_dcb --ext packaged --translateCats ../Plots/cats.json &
    done
    ...
    do
    	...
    	python3 RunPlotter.py --procs $proc --cats "all" --years ${YEAR} --ext packaged --outdir /eos/user/r/rgargiul/www/width_pdf/plots_dcb &
    done
    ...
    do
        ...
        do
            ...
            #python3 RunPlotter.py --procs $proc --cats $cat --years ${YEAR} --ext packaged --outdir /eos/user/r/rgargiul/www/width_pdf/plots_dcb &
        done
    done
