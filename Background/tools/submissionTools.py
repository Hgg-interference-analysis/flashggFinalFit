  ...
  if _opts['batch'] == "condor":
    ...
    if( _opts['mode'] == "fTestParallel" ):
      for cidx in range(_opts['nCats']):
        ...
        _cmd = "%s/runBackgroundScripts.sh -i %s -p %s -f %s --ext %s --catOffset %g --intLumi %s --year %s --batch %s --queue %s --sigFile %s --isData --fTest --pdir %s --addInt %s --intFile %s"%(bwd__,_opts['dataFile'],_opts['procs'],c,_opts['ext'],co,_opts['lumi'],_opts['year'],_opts['batch'],_opts['queue'],_opts['signalFitWSFile'],_opts['plotdir'],_opts["addInt"], _opts["intFile"])
        _f.write("  %s\n"%_cmd)
        _f.write("fi\n")
  ...
  if (_opts['batch'] == "IC")|(_opts['batch'] == "SGE")|(_opts['batch'] == "Rome")|(_opts['batch'] == "local" ):
    ...
    if _opts['mode'] == "fTestParallel":
      for cidx in range(_opts['nCats']):
        ...
        writePreamble(_f)
        _cmd = "%s/runBackgroundScripts.sh -i %s -p %s -f %s --ext %s --catOffset %g --intLumi %s --year %s --batch %s --queue %s --sigFile %s --isData --fTest --addInt %s --intFile %s"%(bwd__,_opts['dataFile'],_opts['procs'],c,_opts['ext'],co,_opts['lumi'],_opts['year'],_opts['batch'],_opts['queue'],_opts['signalFitWSFile'], _opts["addInt"], _opts["intFile"])
        _f.write("%s\n"%_cmd)
        _f.close()
