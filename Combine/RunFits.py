...
for fidx in range(len(fits)):
  ...
  # For 2D scan: fix other pois to 0
    ...
    if opt.batch == 'local':
      fitcmd = "combine -M MultiDimFit -m 125.38 %s -P %s -P %s --floatOtherPOIs 1 %s -n _%s_%s --algo grid --points %s --alignEdges 1 %s %s %s"%(d_opts,_fitpois[0],_fitpois[1],exp_opts,_name,_poisStr,_points.split(":")[0],_fit_opts,pdf_opts,common_opts)
    else:
  ...
  # For 2D scan: fix other pois to 0
    ...
    _poisStr = "%s_vs_%s"%(_fitpois[0],_fitpois[1])
    if opt.batch == 'local':
      fitcmd = "combine -M MultiDimFit -m 125.38 %s -P %s -P %s --floatOtherPOIs 0 %s -n _%s_%s --algo grid --points %s --alignEdges 1 %s %s %s"%(d_opts,_fitpois[0],_fitpois[1],exp_opts,_name,_poisStr,_points.split(":")[0],_fit_opts,pdf_opts,common_opts)
    else:
