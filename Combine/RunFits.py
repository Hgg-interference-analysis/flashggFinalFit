...
for fidx in range(len(fits)):
  ...
  if _fit.split(":")[0] == "bestfit":
    ...
    for poi in _fitpois:
      if opt.batch == 'local':
        fitcmd = "combine -M MultiDimFit %s %s -n _%s_%s -P %s %s %s %s"%(d_opts,exp_opts,_name,poi,poi,_fit_opts,pdf_opts,common_opts)
      else:
  ...
  if _fit.split(":")[0] == "singles":
    ...
    for poi in _fitpois:
      if opt.batch == 'local':
        fitcmd = "combine -M MultiDimFit %s --floatOtherPOIs 1 %s -n _%s_%s -P %s --algo singles %s %s %s"%(d_opts,exp_opts,_name,poi,poi,_fit_opts,pdf_opts,common_opts)
      else:
  ...
  if _fit.split(":")[0] == "fixed":
    ...
    for poi in _fitpois:
      if opt.batch == 'local':
        fitcmd = "combine -M MultiDimFit %s --floatOtherPOIs 1 %s -n _%s_%s -P %s --algo fixed %s %s %s"%(d_opts,exp_opts,_name,poi,poi,_fit_opts,pdf_opts,common_opts)
      else:
  ...
  if _fit.split(":")[0] == "AsymptoticLimit":
    ...
    for poi in _fitpois:
      if opt.batch == 'local':
        fitcmd = "combine -M AsymptoticLimits %s %s -n _%s_%s --redefineSignalPOI %s %s %s %s"%(d_opts,exp_opts,_name,poi,poi,_fit_opts,pdf_opts,common_opts)
      else:
  ...
  # For 1D scan when profiling other pois
    ...
    for poi in _fitpois:
      if opt.batch == 'local':
        fitcmd = "combine -M MultiDimFit %s --floatOtherPOIs 1 %s -n _%s_%s -P %s --algo grid --points %s --alignEdges 1 %s %s %s"%(d_opts,exp_opts,_name,poi,poi,_points.split(":")[0],_fit_opts,pdf_opts,common_opts)
      else:
  ...
  # For 1D scan when fixing other pois
    ...
    for poi in _fitpois:
      if opt.batch == 'local':
        fitcmd = "combine -M MultiDimFit %s --floatOtherPOIs 0 %s -n _%s_%s -P %s --algo grid --points %s --alignEdges 1 %s %s %s"%(d_opts,exp_opts,_name,poi,poi,_points.split(":")[0],_fit_opts,pdf_opts,common_opts)
      else:
  ...
  # For 2D scan: fix other pois to 0
    ...
    if opt.batch == 'local':
      fitcmd = "combine -M MultiDimFit -m 125 %s -P %s -P %s --floatOtherPOIs 1 %s -n _%s_%s --algo grid --points %s --alignEdges 1 %s %s %s"%(d_opts,_fitpois[0],_fitpois[1],exp_opts,_name,_poisStr,_points.split(":")[0],_fit_opts,pdf_opts,common_opts)
    else:
  ...
  # For 2D scan: fix other pois to 0
    ...
    _poisStr = "%s_vs_%s"%(_fitpois[0],_fitpois[1])
    if opt.batch == 'local':
      fitcmd = "combine -M MultiDimFit -m 125 %s -P %s -P %s --floatOtherPOIs 0 %s -n _%s_%s --algo grid --points %s --alignEdges 1 %s %s %s"%(d_opts,_fitpois[0],_fitpois[1],exp_opts,_name,_poisStr,_points.split(":")[0],_fit_opts,pdf_opts,common_opts)
    else:
